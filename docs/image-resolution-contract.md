...
- `gpt-image-2.5-flare` and `gpt-image-2.5-sunburst` use the third-party `/v1/chat/completions` image conversion route confirmed by the user for Magic. Requests retain the exact model ID and send `messages` with text and optional `image_url` data URLs, plus `stream: false`, following the provider's [Chat Completions format](https://docs.magic666.top/api-reference/texts/chat-completions-non-stream). They do not send the Images API `prompt`, `n`, `size`, or `response_format` fields. Base URLs with or without a trailing `/v1` are accepted for this route.
- GPT Image 2.5 keeps the existing resolution selector, but conveys the chosen aspect ratio and mapped size in the chat prompt. Actual output dimensions depend on the proxy's image conversion. Returned Markdown URLs, data URLs, base64 content, and structured chat image attachments are accepted; explicit image attachments take priority over links in accompanying text. A text-only reply is reported as an error. This is a third-party adapter, not OpenAI's native GPT Image API contract.
- The GPT Image 2.5 display characteristics follow the official [Flare](https://developers.openai.com/api/docs/models/gpt-image-2.5-flare) and [Sunburst](https://developers.openai.com/api/docs/models/gpt-image-2.5-sunburst) model descriptions.
