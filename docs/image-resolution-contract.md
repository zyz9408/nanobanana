...
- The image model selector is intentionally allowlisted to three visible choices: `Gemini 3 Image Pro`, `Gemini 3.1 Image Fast`, and `GPT Image 2`. The remote model reader applies the same exact allowlist and hides other image models such as Gemini 2.5, GPT Image 2 Pro, and resolution-suffixed Gemini variants like `gemini-3.1-flash-image-preview-2k`.
